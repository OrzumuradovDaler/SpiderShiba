    ...
    for (let item of data) {
        ...
        btn_favorite.onclick = () => {
            ...
            if (cart.includes(item.image)) {
                ...
                cart = cart.filter(el => el !== item.image)
                menu__item.remove()
            } else {
                btn_favorite.classList.add('fav_act')
                cart.push(item.image)

                if(btn_favorite.classList.contains('favarite') !== true){
                 menu__item = document.createElement('div')
                ...
                menu__item.append(item_img)
                menu__item.append(item_price)
                menu__item.append(button_cont)

                item_price.append(item_p)
                item_price.append(item_span)

                button_cont.append(button_minus)
                button_cont.append(button_span)
                button_cont.append(button_plus)
                menu__box.append(menu__item)

                menu__item.classList.add('menu__item')
                item_img.classList.add('image1')
                item_price.classList.add('pricee')
                button_cont.classList.add('button_cont')
                button_minus.classList.add('minus')
                button_plus.classList.add('plus')
                button_span.classList.add('spaner')

                button_span.innerHTML = 1
                button_plus.innerHTML = '+'
                button_minus.innerHTML = '-'

                item_img.src = item.image
                item_p.innerHTML =item.price
                item_span.innerHTML = '$'
                button_plus.onclick = () =>{
                    button_span.innerHTML++
                    let num = Number(item_p.innerHTML) + item.price
                    item_p.innerHTML = Math.floor(num)
                }
                button_minus.onclick = () =>{
                    button_span.innerHTML--
                    let num = Number(item_p.innerHTML) - item.price
                    item_p.innerHTML = Math.floor(num)
                    if(button_span.innerHTML === '0'){
                        menu__item.remove()
                        btn_favorite.classList.remove('fav_act')
                        cart = cart.filter(el => el !== item.image)
                    }else{
